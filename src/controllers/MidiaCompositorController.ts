...
import MidiaCompositor from "../models/MidiaCompositorModel";

class MidiaCompositorController {
  /**
   * Cria uma nova associação entre mídia e compositor
  ...
  async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      const associacaoCriada = await MidiaCompositor.create(novaAssociacao);
  ...
  /**
   * Lista todas as associações entre mídia e compositor
   * @param req Request com os parâmetros da requisição (opcional)
   * @param res Response para enviar a resposta HTTP
   */
  async list(req: Request, res: Response): Promise<void> {
    try {
      const associacoes = await MidiaCompositor.findAll();
  ...
  /**
   * Busca uma associação entre mídia e compositor por ID
  ...
  async findById(req: Request, res: Response): Promise<void> {
    const { id } = req.params;
    try {
      const associacao = await MidiaCompositor.findByPk(id);
      if (associacao) {
  ...
  /**
   * Atualiza uma associação existente entre mídia e compositor por ID
  ...
  async update(req: Request, res: Response): Promise<void> {
    ...
    try {
      const associacaoAtualizada = await MidiaCompositor.update(
        novosDadosAssociacao,
        {
          where: { id_midia_compositor: id },
          returning: true,
        }
      );
  ...
  /**
   * Exclui uma associação entre mídia e compositor por ID
  ...
  async delete(req: Request, res: Response): Promise<void> {
    const { id } = req.params;
    try {
      const associacaoExcluida = await MidiaCompositor.destroy({
        where: { id_midia_compositor: id },
      });
...
export default new MidiaCompositorController();
